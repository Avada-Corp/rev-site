...
export const superAdminGuard = (
  ...
  return store.pipe(
    ...
    switchMap((isLogged) => {
      ...
      // Если пользователь авторизован, проверяем его права суперадминистратора
      return store.pipe(
        ...
        map((user) => {
          if (user?.userRole === UserRole.SuperAdmin) {
            ...
            // Если пользователь не суперадминистратор, проверяем наличие обычных админских прав
            if (user?.userRole === UserRole.Admin) {
              // Если пользователь обычный админ, перенаправляем на админскую панель
              router.navigateByUrl('/admin');
            } else {
              // Если пользователь вообще не имеет админских прав, перенаправляем на главную
